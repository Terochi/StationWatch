...
async def get_dashboard():
    try:
        ...
        return HTMLResponse(content="<h1>index.html not found!</h1><p>Ensure index.html is in the same directory as server.py.</p>", status_code=404)
...
async def register_device(form: RegistrationForm):
    with get_db() as db:
        db.execute("UPDATE devices SET name = ?, owner = ?, category = ?, is_registered = 1, is_monitored = 1 WHERE id = ?", 
...
async def toggle_pc_monitor(pc_id: str):
    with get_db() as db:
        db.execute("UPDATE pcs SET is_monitoring = CASE WHEN is_monitoring = 1 THEN 0 ELSE 1 END WHERE id = ?", (pc_id,))
        db.commit()
    await manager.broadcast({"type": "pc_updated"})
    return {"status": "success"}
...
async def toggle_device_ignore(device_id: str):
    with get_db() as db:
        db.execute("UPDATE devices SET is_ignored = CASE WHEN is_ignored = 1 THEN 0 ELSE 1 END WHERE id = ?", (device_id,))
        db.commit()
    await manager.broadcast({"type": "hardware_changed"})
    return {"status": "success"}
...
async def debounce_window_check(pc_id, target_exe):
    ...
    with get_db() as db:
        pc = db.execute("SELECT current_exe, preferred_exe, is_monitoring, name FROM pcs WHERE id = ?", (pc_id,)).fetchone()
...
async def receive_heartbeat(payload: dict):
    ...
    with get_db() as db:
        db.execute("INSERT INTO pcs (id, last_seen) VALUES (?, ?) ON CONFLICT(id) DO UPDATE SET last_seen=excluded.last_seen", 
    ...
    with get_db() as db:
        if event_type == "window_changed":
            ...
            for dev in connected:
                ...
                """, (dev["id"], pc_id, dev["name"], dev.get("category","Other"), dev.get("manufacturer","Generic"), dev.get("vid"), dev.get("pid")))
            
            for dev in disconnected:
                db.execute("UPDATE devices SET is_connected = 0 WHERE id = ? AND last_connected_pc = ?", (dev["id"], pc_id))
                
                pc = db.execute("SELECT is_monitoring, name FROM pcs WHERE id = ?", (pc_id,)).fetchone()
                device_info = db.execute("SELECT name, is_registered, is_ignored FROM devices WHERE id = ?", (dev["id"],)).fetchone()
                
                if pc and pc["is_monitoring"] and device_info and device_info["is_registered"] and not device_info["is_ignored"]:
                    send_ntfy(
